...
PATRON_RUIDO_ADMINISTRATIVO = re.compile(
    ...
    r"SENA\s+CAQUET|DOCUMENTO\s+DE\s+IDENTIDAD\s+FACILITADO|"
    r"INSCRIPCI[OÓ]N.*MATR[IÍ]CULA|PARA\s+PROCESO\s+DE|"
    # Patrón ciudad-departamento fusionado por OCR (ej: FLORENCIA-CAQUETÁ, ARMENIA-QUINDÍO)
    r"FLORENCIA[\s\-]?CAQUET|ARMENIA[\s\-]?QUIND|NEIVA[\s\-]?HUILA|TUNJA[\s\-]?BOYAC|"
